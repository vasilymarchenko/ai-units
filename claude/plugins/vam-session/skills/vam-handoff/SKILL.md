...
The failure mode to avoid is a transcript. Nobody needs to know the order in which things were discussed. What the next session needs is the current state of the world, the reasoning behind it, and the shortest path back to productive work.

**TL;DR and Next steps must be actionable in two minutes.** Everything below them is reference material, read on demand. Length is not the enemy; undifferentiated length is.
...
## Check for earlier handoffs on the same topic

Before writing, list the target directory (see *Where to save it*) for files whose topic slug matches. If any exist, read the most recent one, then:

- Open the new document with an explicit **supersedes** line: which parts of the older one are now stale, which are still the best source, and what to read first.
- Do not restate what the older one still covers well — point at it by path.
- Leave the old files alone. Never edit or delete a previous handoff.

A topic that accumulates several handoffs is normal. What costs the next agent real time is three documents with no statement of which one wins.
...
## Suggested skills
Skills the next agent should invoke, and when. Include skills to *avoid* where a
plausible-looking one is wrong for this stack — a wrong skill costs a turn.
```

## Where to save it

Resolve the target directory in this order and stop at the first hit:
...
Finish with one line giving the exact path and how to use it: paste the file, or its contents, into the first message of the new session.

## Setup

One-time, per repository, done by the user — not by this skill:

```sh
echo '/.personal/' >> .git/info/exclude
```

`.git/info/exclude` is preferred over `.gitignore`: it is local to the clone, so a personal scratch directory never appears in a tracked file or in anyone else's diff.
...
- Writing a new handoff on a topic that already has one without saying which parts of the old one are now stale.
- Spending tool calls on anything other than writing the file: no git inspection, no directory auditing, no verifying facts you already established in the conversation.
